...
const titleTransition = { delay: 0.15, duration: 0.5, ease: [0.44, 0, 0.56, 1] };
const ctaTransition = { delay: 1, duration: 0.4, ease: [0.44, 0, 0.56, 1] };
const authorTransition = { type: "spring", bounce: 0.2, delay: 1.4, duration: 0.4 };
const imageTransition = { type: "spring", bounce: 0.2, delay: 1.6, duration: 0.6 };
...
function ResponsiveImage({ image, className, alt = "", sizes, style }) {
  const value = normalizeImage(image);
  if (!value) return <div className={`${className} hero-missing-image`} style={style} aria-label="Image requise" />;
  return (
    <img
      className={className}
      src={value.src}
      srcSet={value.srcSet || value.srcset}
      sizes={sizes}
      alt={value.alt ?? alt}
      style={style}
    />
...
function ChevronRight({ className = "" }) {
  return (
    <svg className={className} viewBox="0 0 24 24" role="presentation" aria-hidden="true">
      <path d="M 0 12 L 6 6 L 0 0" fill="transparent" stroke="rgb(0,0,0)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" transform="translate(9 6)" />
    </svg>
  );
}
  ...
  return (
    ...
    >
      <motion.span
        ...
        variants={primary ? { rest: { background: "linear-gradient(90deg, rgb(78, 125, 250) 0%, rgb(1, 71, 255) 100%)" }, hover: { background: "linear-gradient(161deg, rgb(67, 108, 222) 0%, rgb(36, 77, 181) 100%)" } } : undefined}
        transition={spring}
      >
        <span className="hero-cta-label">{children}</span>
        {primary && (
          <motion.span className="hero-cta-avatar-wrap" variants={{ rest: { rotate: 0 }, hover: { rotate: 10 } }} transition={spring}>
            <img
...
function useIsPhone() {
  const [isPhone, setIsPhone] = React.useState(() => typeof window !== "undefined" && window.matchMedia("(max-width: 809px)").matches);
  React.useEffect(() => {
...
export default function HeroCaseStudy({
  ...
  const profileImage = normalizeImage(photoDeProfil);
  ...
  return (
    <section className={`hero-case-study ${className}`.trim()}>
          ...
          <div className="hero-copy-column">
            <div className="hero-copy-stack">
              <nav className="hero-breadcrumb" aria-label="Fil d’Ariane">
                <BreadcrumbLink href={realisationsHref}>Réalisations</BreadcrumbLink>
              ...
              <div className="hero-heading-wrap">
                ...
                <h1 className="hero-title hero-title-mobile">{title}</h1>
              </div>
              ...
              >
                <div className="hero-offers-cta">
                  <CTA variant="secondary" href={offersHref}>Voir nos offres</CTA>
                </div>
                <div className="hero-call-cta">
                  <CTA variant="primary" href={callHref} fullWidth>Réserver un appel</CTA>
                </div>
              </motion.div>

              <motion.div
                className="hero-author"
                initial={isPhone ? false : { opacity: 0.001, y: 36 }}
                animate={{ opacity: 1, y: 0 }}
                transition={isPhone ? undefined : authorTransition}
              >
                <span className="hero-author-photo-wrap">
                  <ResponsiveImage
                    image={profileImage}
                    className="hero-author-photo"
                    sizes="78px"
                    alt={nomDeLaPersonne ? `Photo de ${nomDeLaPersonne}` : ""}
                  />
                </span>
                <div className="hero-author-copy">
                  <p className="hero-author-name">{nomDeLaPersonne}</p>
                  <p className="hero-author-role">{fonctionDeLaPersonne}</p>
                </div>
              </motion.div>
            </div>
          </div>
      ...
      <svg className="hero-deco hero-deco-right" width="923" height="822" viewBox="-37 -37 923 822" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      ...
      <svg className="hero-deco hero-deco-left" width="661" height="870" viewBox="-37 -37 661 870" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
