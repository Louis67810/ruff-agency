...
function ProcessAnimation({ step }: { step: Step }) {
  ...
  useEffect(() => {
    if (window.matchMedia("(max-width: 809px)").matches || !canvasRef.current) return;

    let player: DotLottie | undefined;
    let cancelled = false;

    void import("@lottiefiles/dotlottie-web").then(({ DotLottie: DotLottiePlayer }) => {
  ...
  return (
    <div className={styles.visual} aria-hidden="true">
      <canvas
        ...
        style={{ transform: `translate3d(${step.transform.x}px, ${step.transform.y}px, 0) scale(${step.transform.scale})` }}
      />
    </div>
  );
}
    ...
    <section className={styles.section} aria-labelledby="process-title">
      <div className={styles.inner}>
        <div className={styles.heading}>
          <h2 id="process-title">Un processus de création simple et efficace pour des projets uniques</h2>
...
function CardReveal({ children, delay }: { children: ReactNode; delay: number }) {
  ...
  return <div ref={ref} className={`${styles.cardReveal} ${visible ? styles.isVisible : ""}`} style={{ "--card-delay": `${delay}s` } as CSSProperties}>{children}</div>;
}
...
export function ExactProcessCardGrid() {
  ...
  useEffect(() => {
    ...
    const syncLayout = () => {
      const nextVisibleCards = window.innerWidth <= 809 ? 1 : query.matches ? 2 : 3;
      setVisibleCards(nextVisibleCards);
      setActiveIndex((current) => Math.min(current, steps.length - nextVisibleCards));
    };
    syncLayout();
  ...
  const isCarousel = visibleCards < steps.length;

  return <div className={styles.carousel} data-carousel={isCarousel || undefined}>
    <div
      ref={viewportRef}
    ...
    >
      <div className={styles.grid} data-active-step={activeIndex}>
        {steps.map((step, index) => <CardReveal key={step.label} delay={0.8 + index * 0.35}>
          <article className={styles.card}>
            <span className={styles.label} style={{ backgroundColor: step.color }}>{step.label}</span>
    ...
    <div className={styles.navigation} aria-label="Navigation entre les étapes">
      <button className={styles.arrow} type="button" onClick={() => goTo(activeIndex - 1)} disabled={activeIndex === 0} aria-label="Étape précédente">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m15.75 19.5-7.5-7.5 7.5-7.5" /></svg>
      </button>
      <button className={styles.arrow} type="button" onClick={() => goTo(activeIndex + 1)} disabled={activeIndex === maxIndex} aria-label="Étape suivante">
