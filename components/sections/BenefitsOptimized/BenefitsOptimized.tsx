...
type CaseVariant = "b0Pu6QCvO" | "V5Wh7855Z" | "N4z3YzW4M" | "lxULhvTaN" | "lScI4MQ5L" | "E_Q5KywxY";
...
function AnimatedHeading() {
  const muted = ["Tout", "ce", "dont", "vous", "avez", "besoin", "pour"];
  const strong = ["faire", "croître", "votre", "business"];
      ...
      <span className="headingStrong">
        {strong.map((word, index) => (
          <React.Fragment key={`strong-${word}-${index}`}>
            <span className="headingWord">{word}</span>{index < strong.length - 1 ? " " : ""}
          </React.Fragment>
        ))}
      </span>
    </h2>
...
function CaseInteractive() {
  ...
  const isPhoneVariant = variant === "lxULhvTaN" || variant === "lScI4MQ5L" || variant === "E_Q5KywxY";

  useEffect(() => {
    const query = window.matchMedia("(max-width: 809px)");
    const sync = () => setVariant((current) => {
      const phone = current === "lxULhvTaN" || current === "lScI4MQ5L" || current === "E_Q5KywxY";
  ...
  const selectCase = (side: "left" | "center" | "right") => {
    if (isPhoneVariant) {
      setVariant(side === "left" ? "lScI4MQ5L" : side === "right" ? "E_Q5KywxY" : "lxULhvTaN");
    } else {
      setVariant(side === "left" ? "V5Wh7855Z" : side === "right" ? "N4z3YzW4M" : "b0Pu6QCvO");
    }
  };

  return (
<div className={`framer-zCdZt framer-gh94k5 ${CASE_CLASS[variant]}`} style={{ width: "100%", height: isPhoneVariant ? "auto" : "100%" }}>
  <div className="framer-xzorfx">
    ...
    <div className="framer-2dovbc">
      <button type="button" aria-label="Style gauche" className="framer-1rhbe3q" onClick={() => selectCase("left")}><SvgSlot className="framer-1nt377z" /></button>
      <button type="button" aria-label="Style centré" className="framer-ewztt2" onClick={() => selectCase("center")}><SvgSlot className="framer-97g3ku" /></button>
      <button type="button" aria-label="Style droite" className="framer-1rtzczk" onClick={() => selectCase("right")}><SvgSlot className="framer-c1pbid" /></button>
...
function BookingCta({ href }: { href: string }) {
  return (
    <a className="benefitsCta" href={href}>
      <span className="benefitsCtaInner">
        <span className="benefitsCtaText">Réserver un appel</span>
        <span className="benefitsCtaAvatarWrap"><img className="benefitsCtaAvatar" src="https://framerusercontent.com/images/2MtWvpxSPxrD8xwJa7XBZm2R8PE.jpg?lossless=1&width=693&height=693" alt="Photo de profil de Louis Staub" /></span>
      </span>
    </a>
  );
}

export default function BenefitsOptimized({ bookingHref = "#", className = "" }: BenefitsOptimizedProps) {
        ...
        <div className="framer-99078z">
          <div className="framer-yh70c">
            <div className="framer-190ikna">
              <div ref={headingReveal.ref} className={`framer-u844pr ${headingReveal.visible ? "visible" : ""}`}>
                <AnimatedHeading />
              </div>
              <div ref={ctaReveal.ref} className={`framer-1w52cg2-container ${ctaReveal.visible ? "visible" : ""}`}>
        ...
        <div className="framer-3t8agc">
          <div className="framer-br6sth">
            <article className="framer-jp9aoj">
              <div className="framer-1cl97ws"><div className="framer-1db1csp" role="img" aria-label="Illustration d'une transformation de visiteur en client" /></div>
              <div className="framer-19uj0c1">
                <div className="framer-4xlrff"><p className="cardTitle"> Transformez vos visiteurs en clients</p></div>
                <div className="framer-1twlf68"><p className="cardBody" style={{ color: "rgba(0,0,0,.6)" }}>Chaque section de votre landing page est pensée pour guider le regard vers l’action, sans friction.</p></div>
              </div>
            ...
            <article className="framer-ppc8su">
              <div className="framer-1obr3he">
                <div className="framer-16sem8i"><p className="cardTitle" style={{ color: "white" }}>Inspirez confiance</p></div>
                <div className="framer-c01k9r"><p className="cardBody" style={{ color: "rgba(255,255,255,.6)" }}>Un rendu net et cohérent qui rassure dès l’arrivée sur la page.</p></div>
              </div>
          ...
          <div className="framer-1l694m3">
            <div className="framer-1d4gv6m">
              <div className="framer-o0wzvt-container"><CaseInteractive /></div>
              <article className="framer-3iqtzm">
                <div className="framer-1n7abvt">
                  <div className="framer-jzz6ry"><p className="cardTitle" style={{ color: "white" }}>Devenez le résultat numéro 1 dans les recherches</p></div>
                  <div className="framer-okvcxv"><p className="cardBody" style={{ color: "rgba(255,255,255,.8)" }}>La structure de la page, les titres et les contenus sont pensés pour améliorer la lisibilité de votre offre par les moteurs de recherche.</p></div>
                </div>
            ...
            <div className="framer-k6a6w5">
              <article className="framer-kbhskc">
                <div className="framer-5ev6jp" role="img" aria-label="Illustration de structure de page" />
                <div className="framer-1sihc94" />
                <div className="framer-nf4h8p">
                  <div className="framer-1dhw1yd"><p className="cardTitle">Structurer votre offre</p></div>
                  <div className="framer-57kvaq"><p className="cardBody structureBody" style={{ color: "rgba(0,0,0,.8)" }}>Votre proposition devient lisible, hiérarchisée et facile à choisir.</p></div>
                </div>
              </article>
            </div>
          </div>
