...
const DEFAULT_COPY = {
  about:
    "Transformez vos visiteurs en clients et renforcez votre crédibilité grâce à une landing page premium, optimisée pour la conversion.",
  review:
    "Lorem ipsum dolor sit amet. Nam voluptas alias vel tenetur rerum aut adipisci eaque. Non assumenda voluptas sed maiores voluptate ut facere error. Qui quidem officia est reprehenderit necessitatibus aut quod molestias qui ullam quaerat.",
  personName: "Louis",
  ...
  solutionSubtitle:
    "Nous avons conçu un site premium, pensé comme une vitrine artistique et professionnelle.",
  ...
  resultsSubtitle:
    "Ce nouveau site a permis à Scall de franchir un cap dans sa communication.",
...
function Availability({ english = false }) {
  ...
  const dateData = useMemo(() => {
    const now = new Date();
    const month = now.toLocaleDateString(english ? "en-US" : "fr-FR", {
      month: "long",
    });
  ...
  return (
    <motion.div
      className="cprs-availability"
      initial={{ opacity: 0.001, y: 35 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ type: "spring", bounce: 0.2, duration: 0.4 }}
    >
      <div className="cprs-lottie-crop">
        <div className="cprs-lottie" ref={ref} />
      </div>
      <div className="cprs-availability-text">
        <span className="cprs-number">{dateData.number}</span>
        <span>
          {english
            ? `spots left for ${dateData.month}`
            : `places restantes pour ${dateData.month}`}
        </span>
      </div>
    </motion.div>
  );
...
function ChevronRight() {
  return (
    <svg className="cprs-chevron" viewBox="0 0 24 24" aria-hidden="true">
      <path
...
function SiteButton({ href, english = false }) {
  if (!href) return null;
  return (
    <a className="cprs-site-cta" href={href}>
      <span>{english ? "View live site" : "Voir le site en direct"}</span>
      <ChevronRight />
    </a>
  );
}
function CallButton({ href = "#", english = false }) {
  return (
    <a className="cprs-call-shell" href={href}>
      <span className="cprs-call-inner">
...
function InfoRow({ type, children }) {
  return (
    <div className={`cprs-info-row cprs-${type}`}>
      <div className="cprs-info-icon">
        {type === "challenge" ? (
          <XIcon />
        ) : type === "solution" ? (
          <RIcon />
      ...
      <div className="cprs-info-title">{children}</div>
    </div>
  );
}
...
export default function ContentPageRealisationsSlug({
  ...
  const resultItems = [result1, result2, result3, result4].filter(
    (x, i) => i < 3 || Boolean(x),
  );
      ...
      <div className="cprs-layout">
        <aside className="cprs-sidebar">
          <div className="cprs-about">
            {logoImage ? (
              <img
                ...
                sizes="81px"
                alt={logoImage.alt || "Logo du projet"}
              />
            ) : null}
            <AnimatedLines className="cprs-about-copy">{about}</AnimatedLines>
            <SiteButton href={siteHref} english={english} />
          </div>

          {review ? (
            <div className="cprs-review-card">
              <motion.div
                className="cprs-review-person"
              ...
              >
                {profile ? (
                  <img
                    className="cprs-profile"
                    src={profile.src}
                    srcSet={profile.srcSet}
                    sizes="63px"
                    alt={profile.alt || `Portrait de ${personName}`}
                  />
                ) : (
                  <div className="cprs-profile cprs-profile-empty" />
                )}
                <div className="cprs-person-copy">
                  <div className="cprs-person-name">{personName}</div>
                  <div className="cprs-person-role">{personRole}</div>
                </div>
              </motion.div>
          ...
          <div className="cprs-blue-card">
            ...
            <CallButton href={callHref} english={english} />
            <div className="cprs-blue-decorations">
              <img
                className="cprs-decor-1"
                src={CTA_DECORATION_1.src}
                srcSet={CTA_DECORATION_1.srcSet}
                sizes="293px"
                alt="Illustration décorative du projet"
              />
              <img
                className="cprs-decor-2"
                src={CTA_DECORATION_2.src}
                srcSet={CTA_DECORATION_2.srcSet}
                sizes="206px"
                alt="Illustration décorative du projet"
              />
            </div>
        ...
        <div className="cprs-main">
          <div className="cprs-content">
            <section className="cprs-copy-section">
              <div className="cprs-section-intro">
                <h2>{english ? "The challenge" : "Le challenge"}</h2>
                <p>{challengeSubtitle}</p>
              </div>
              <div className="cprs-list cprs-challenge-list">
                {challengeItems.map((item, i) =>
                  !isPhone || i > 1 || Boolean(item) ? (
                    <InfoRow type="challenge" key={i}>
                      {item}
                    </InfoRow>
                  ) : null,
                )}
            ...
            <section className="cprs-copy-section">
              <div className="cprs-section-intro">
                <h2>{english ? "The solution" : "La solution"}</h2>
                <p>{solutionSubtitle}</p>
              </div>
              <div className="cprs-list cprs-solution-list">
                {solutionItems.map((item, i) => (
                  <InfoRow type="solution" key={i}>
                    {item}
                  </InfoRow>
                ))}
              </div>
            ...
            <section className="cprs-copy-section">
              <div className="cprs-section-intro">
                <h2>{english ? "The results" : "Les résultats"}</h2>
                <p>{resultsSubtitle}</p>
              </div>
              <div className="cprs-list cprs-results-list">
                {resultItems.map((item, i) => (
                  <InfoRow type="result" key={i}>
                    {item}
                  </InfoRow>
                ))}
              </div>
