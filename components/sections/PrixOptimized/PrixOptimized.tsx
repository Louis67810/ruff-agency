...
function thumbPosition(value: number, min: number, max: number, trackWidth: number) {
  const thumbWidth = 28;
  if (max <= min) return thumbWidth / 2;
  return ((value - min) / (max - min)) * (trackWidth - thumbWidth) + thumbWidth / 2;
}
...
export default function PrixOptimized({
  ...
  landingDescription =
    'Idéal pour un produit, un service ou un objectif unique, une page optimisée pour convertir.',
  siteTitle = 'Site Internet',
  siteDescription =
    'Idéale pour ceux qui veulent scaler et ont une audience plus large.',
  ...
  const [activeUpsells, setActiveUpsells] = useState<Set<UpsellId>>(() => new Set());
  ...
  const pageDescription = pages === 1 ? landingDescription : siteDescription;
  const fillPercentage = ((rawValue - 1) / (safeMaxPages - 1)) * 100;
  const badgeLeft = thumbPosition(rawValue, 1, safeMaxPages, trackWidth);
  const ticks = useMemo(() => Array.from({ length: safeMaxPages }), [safeMaxPages]);
  const leftFeatures = FEATURES.filter((_, index) => index % 2 === 0);
  const rightFeatures = FEATURES.filter((_, index) => index % 2 !== 0);
  const ticker = tickerImages.length > 0 ? tickerImages : Array<string | null>(8).fill(null);
  ...
  return (
    <section className={`prx-section ${className}`.trim()}>
      <div className="prx-sectionInner">
        <h2 className="prx-sectionTitle">Des tarifs clairs, sans surprise</h2>
        ...
        <div className="prx-cardShell">
          <div className="prx-card">
            <div className="prx-leftPanel">
              ...
              <div className="prx-optionsLabel">Options supplémentaires :</div>
                ...
                {UPSELLS.map((upsell) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="prx-upsellPrice">+{upsellPrices[upsell.id]}€</div>
                    </button>
                  );
                })}
              </div>
              ...
              <div className="prx-sliderModule">
                <div className="prx-sliderInfo">{sliderInfoText}</div>
                <div className="prx-sliderControl">
                  ...
                  <div className="prx-trackBackground" aria-hidden="true">
                    ...
                    <div className="prx-trackFill" style={{ width: `${fillPercentage}%` }} />
                  </div>
              ...
              <div className="prx-priceArea">
                <div className="prx-total">{displayedPrice.toLocaleString('fr-FR')}€</div>
            ...
            <div className="prx-rightPanel">
              <h3 className="prx-cardTitle">Ce qui est inclus :</h3>
        ...
        <div className="prx-guarantee">
          <p className="prx-guaranteeTitle">Satisfait ou 100% Garantit</p>
          <p className="prx-guaranteeDescription">
            Nous perfectionnons votre site jusqu’à ce qu’il vous convienne entièrement, ou nous vous
