...
const DEFAULT_TITLE = "On crée des World-class landing pages";
...
const portfolio = [
  ...
  },
  {
    ...
    alt: "Hero section dark mode pour un graphiste indépendant",
  },
  {
...
export function ClientsSatisfaits() {
  return (
...
export function Availability({ english = false }: { english?: boolean }) {
...
function AnimatedSubtitle({ text }: { text: string }) {
  ...
  return (
    <p className="h2-subtitle" ref={ref}>
      {words.map((word, index) => (
        <React.Fragment key={`${word}-${index}`}>
          <span className="h2-subtitle-word" data-subtitle-word>
            {word}
          </span>
          {index < words.length - 1 ? " " : null}
        </React.Fragment>
...
function CTA({
  ...
  return (
    <a
      className={`h2-cta ${kind === "primary" ? "h2-cta-primary" : "h2-cta-secondary"}`}
      href={href}
    >
          ...
          >
            {portfolio.map((item, index) => (
              <img
                ...
                src={item.src}
                alt={group === 0 ? item.alt : "Illustration décorative de la réalisation"}
                key={`${group}-${item.src}`}
...
function BackgroundShapes() {
  return (
    <div className="h2-background-shapes" aria-hidden="true">
      <svg
        className="h2-shape h2-shape-right"
      ...
      <svg
        className="h2-shape h2-shape-left"
...
export default function Hero2Optimized({
  ...
  secondaryCtaHref = localizeHref(secondaryCtaHref, locale);
  primaryCtaHref = localizeHref(primaryCtaHref, locale);
  title = translateText(title, locale);
  subtitle = translateText(subtitle, locale);
  secondaryCtaLabel = translateText(secondaryCtaLabel, locale);
  primaryCtaLabel = translateText(primaryCtaLabel, locale);
  return (
          ...
          <div className="h2-left-section">
            <div className="h2-top-content">
              ...
              <div className="h2-cta-row">
                <CTA
                  kind="secondary"
                  href={secondaryCtaHref ?? realisationsHref}
                >
                  {secondaryCtaLabel}
                </CTA>
                <CTA kind="primary" href={primaryCtaHref ?? bookingHref}>
                  {primaryCtaLabel}
                </CTA>
              </div>
