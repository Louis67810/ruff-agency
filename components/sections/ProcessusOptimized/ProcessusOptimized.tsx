...
function Reveal({
  ...
  return (
    <div
      ref={ref}
      className={`processus-reveal processus-reveal--${kind} ${visible ? "is-visible" : ""} ${className}`}
      style={{ "--processus-delay": `${delay}s` } as CSSProperties}
    >
      {children}
    </div>
  );
...
function ProjectCta({ href, english }: { href: string; english: boolean }) {
  return (
    <a className="processus__cta" href={href}>
      <span className="processus__ctaInner">
        <span className="processus__ctaText">
          {english ? "Start my project now" : "Je lance mon projet maintenant"}
        </span>
      </span>
    </a>
  );
...
export default function ProcessusOptimized({
  ...
  projectHref = localizeHref(projectHref, english ? "en" : "fr") || "#";
  return (
    <section className={`processus ${className}`.trim()}>
      ...
      <div className="processus__content">
        <div className="processus__heading">
          <Reveal kind="title" delay={0.1} className="processus__titleReveal">
          ...
          <Reveal kind="rise" delay={0.55}>
            <ProjectCta href={projectHref} english={english} />
          </Reveal>
        </div>
        <svg
        ...
        <div className="processus__panel">
          <ExactProcessCardGrid locale={english ? "en" : "fr"} />
        </div>
      </div>
      <RightLine />
